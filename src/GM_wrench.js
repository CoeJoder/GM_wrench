...
(() => {
	...
	// Logger
	(() => {
		const _menuListeners = [];

		// TODO if possible, dynamically check if menu already exists
		let _isLoggerAlreadyRegistered = false;
		...
		const _defaultOpts = {
			level: "info",
			prefix: GM?.info?.script?.name ?? null,
			registerMenu: true,
			menuLabels: ["trace", "debug", "info"],
			formatter: (msg, level, prefix) =>
				`${prefix !== null ? `[${prefix}] ` : ""}${level !== null ? `[${level.toUpperCase()}] ` : ""}${msg}`,
		};

		const _getValidMenuLabels = (labels) => {
			if (!Array.isArray(labels)) {
				throw new Error("Expected 'labels' to be array");
			}
			const validLabels = [];
			labels.forEach((label) => {
				if (!(label in _logLevels)) {
					throw new Error(`Invalid menu label: ${label}. Expected one of: ${Object.keys(_logLevels).join(", ")}`);
				}
        ...
        /**
         * @classdesc A console logger which can change log-level via the GM menu.
         * @class
         * @memberof GM_wrench
		 * @example
         ...
         * @param {string}        [opts.level=info]           The log-level.  Options: trace, debug, info, warn, error, fatal
         * @param {string | null} [opts.prefix=(script name)] The log message prefix.
         * @param {boolean}       [opts.registerMenu=true]    Whether or not to register a log-level changer in the GM menu.
         * @param {Array<string>} [opts.menuLabels=["trace", "debug", "info"]] The log-levels to show in the GM menu.
         * @param {Function}      [opts.formatter]            The log message formatter.
         */
		const Logger = function ({level, prefix, registerMenu, menuLabels, formatter} = {}) {
			let _level;
			let _formatter;
			...
			Object.defineProperty(this, "level", {
				get: () => _level,
				set: (value) => {
					if (!(value in _logLevels)) {
						throw new Error(`Invalid log level: ${value}. Expected one of: ${Object.keys(_logLevels).join(", ")}`);
					}
					_level = value;
				},
			});
			...
			Object.defineProperty(this, "formatter", {
				...
				set: (value) => {
					if (typeof value !== "function") {
						throw new Error("Invalid formatter. Expected a function");
					}
					_formatter = value;
				},
			});

			// override defaults and validate via setters
			const opts = { ..._defaultOpts, ...arguments[0] };
			this.level = opts.level;
			this.formatter = opts.formatter;
			this.prefix = opts.prefix;

			this._check = function (level) {
				return _logLevels[level] >= _logLevels[this.level];
			};

			this._msgToString = function (msg, level) {
				return this.formatter(msg, level, this.prefix);
			};

			if (opts.registerMenu) {
				_menuListeners.push(this);
				if (!_isLoggerAlreadyRegistered) {
					if (typeof GM?.registerMenuCommand === "undefined") {
						throw new Error(`This userscript must @grant GM.registerMenuCommand`);
					}
					_getValidMenuLabels(opts.menuLabels).forEach(label => {
						GM?.registerMenuCommand(label, () => {
							_menuListeners.forEach(listener => {
								console.log(listener._msgToString(`Setting log level to: ${label.toUpperCase()}`, null));
								listener.level = label;
							});
						});
					});
					_isLoggerAlreadyRegistered = true;
				}
			}
		};

		Logger.prototype.trace = function (msg) {
			if (this._check("trace"))
				console.log(this._msgToString(msg, "trace"));
		};

		Logger.prototype.debug = function (msg) {
			if (this._check("debug"))
				console.log(this._msgToString(msg, "debug"));
		};

		Logger.prototype.info = function (msg) {
			if (this._check("info"))
				console.log(this._msgToString(msg, "info"));
		};

		Logger.prototype.warn = function (msg) {
			if (this._check("warn"))
				console.warn(this._msgToString(msg, "warn"));
		};

		Logger.prototype.error = function (msg) {
			if (this._check("error"))
				console.error(this._msgToString(msg, "error"));
		};

		Logger.prototype.fatal = function (msg) {
			if (this._check("fatal"))
				console.error(this._msgToString(msg, "fatal"));
		};
	...
	const commands = {
		...
		findElements: async function (context, using, value) {
			switch (using) {
				...
				case "linkText": return [...context.querySelectorAll("a")].filter((a) => a.innerText === value);
